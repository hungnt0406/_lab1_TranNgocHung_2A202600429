...
AICB-P1: AI Practicafl Competency Program, Phase 1
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Keep the last 3 conversation turns (6 messages: 3 user + 3 assistant)
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        lines.append(
            f"{trunc(r['prompt']):<42} | {trunc(r['gpt4o_response']):<42} | {trunc(r['mini_response']):<42} | {r['gpt4o_latency']:>14.4f} | {r['mini_latency']:>12.4f}"
        )
    return "\n".join(lines)
